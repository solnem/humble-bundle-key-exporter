    ...
    r = () =>
      [
        ...document.querySelectorAll(
          '.unredeemed-keys-table tbody tr[class=""]'
        ),
      ].map((e) => ({
        platform: e.querySelector(".platform :first-child").title.trim(),
        name: e
          .querySelector(".game-name h4")
          .textContent.trim()
          .replace(/®|™/g, ""),
        bundle: e.querySelector(".game-name p").textContent.trim(),
        bundle_url: e.querySelector(".game-name a").href,
        deadline: "deadline",
        deadline_note: e.querySelector(".custom-instruction strong") != null ? e.querySelector(".custom-instruction strong").textContent.trim() : "no information",
        expiration: "expiration",
        expiration_note: e.querySelector(".expiration-messaging p") != null ? e.querySelector(".expiration-messaging p").textContent.trim() : "no information",
      })),
    c = () =>
      null !==
    ...
    (e.run = () => {
      window.location.pathname.includes("/home/keys")
        ? (async () => {
            ...
            .then((e) => n(e, "humble-bundle-keys.json"))
